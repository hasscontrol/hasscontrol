...
class EntityTypeLightDelegate extends Ui.BehaviorDelegate {
    ...
    function initialize(entId) {
        ...
        var col_mods = Hass.getEntityState(entId)["supported_color_modes"];
    ...
    function onSelect() {
        ...
        if (getCurrentPage() == PAGE_STATE) {
            ...
            pushView(new NumberPickerView(Rez.Strings.Percentage,
                                          entitState.hasKey("brightness") ? entitState["brightness"] * 100 / 255 : 0,
                                          [0, 100, 5]),
            ...
            pushView(new NumberPickerView(Rez.Strings.Mireds,
                                          entitState["color_temp"],
                                          [entitState["min_mireds"],
                                           entitState["max_mireds"],
                                           (entitState["max_mireds"] - entitState["min_mireds"]) / 20]),
...
class EntityTypeLightView extends Ui.View {
    ...
    function onUpdate(dc) {
        ...
        if (currPage == _delegate.PAGE_STATE) {
            ...
            var val = entityState.hasKey("brightness") ? entityState["brightness"] * 100 / 255 : 0;
            ...
            Lay.drawState(dc, entityState["color_temp"] + " " + Ui.loadResource(Rez.Strings.Mireds).toLower());
